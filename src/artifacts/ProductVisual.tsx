...
const DaylightPlane = lazy(() => import("./DaylightPlane").then((m) => ({ default: m.DaylightPlane })));
...
function Plane({ kind }: { kind: ArtifactKind }) {
  ...
  if (kind === "daylight") return <DaylightPlane />;
  return null;
}
...
export function ProductVisual({ kind }: { kind: ArtifactKind }) {
  return (
    <div className="product-visual">
      <Suspense fallback={<div className="plane-fallback" aria-hidden="true" />}>
        <Plane kind={kind} />
      </Suspense>
