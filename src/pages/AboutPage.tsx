...
import { cases, earlier, experience, person, skills } from "@/data/content";
...
export function AboutPage() {
      ...
      <section className="section about-section">
        <div className="wrap-wide about-grid">
          ...
          <div className="about-exp">
            ...
            {experience.map((e, expIndex) => {
              const id = aboutAnchorId(e.company);
              const here = activeId === id;
              return (
                <Reveal key={e.company} inView={expIndex > 0} immediate={expIndex === 0}>
                  <article
                  ...
                  >
                    ...
                    {e.related?.length ? (
                      <div className="exp-cases">
                        <p className="sys">Selected cases from this role</p>
                        <ul className="exp-case-list">
                          {e.related.map((item) => {
                            const study = cases.find((c) => c.slug === item.slug);
                            const proof = study?.metrics[0];
                            return (
                              <li key={item.slug}>
                                <Link
                                  className="text-link"
 ...
 What I bring to agent and market systems - and the craft I hire for.
      ...
      <section className="section about-backers" id="context" tabIndex={-1}>
        <div className="wrap-wide">
          <Reveal>
            <p className="sys">Institutional context</p>
            <h2 className="display">Who backed and acquired the companies I worked at.</h2>
            <p className="lede">
              Investor and owner marks for Quantile, OpenGamma, and related platforms, not personal
              funding or endorsement. The scrolling strip at the top of every page is the same set.
              Useful context for the employers above; the work itself is on Work.
            </p>
          </Reveal>
        </div>
      </section>
