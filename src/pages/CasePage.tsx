...
import { canEmbedProduct, ProductPreview } from "@/components/ProductPreview";
...
type StoryBeat = "broken" | "decision" | "control" | "impact" | "rest";
...
function CaseStudyView({ study, index }: { study: CaseStudy; index: number }) {
  ...
  useEffect(() => {
    ...
    // Empty hash: keep TOC on the first beat, but leave the diagram on its teaching default
    // (rest) until the visitor scrolls into a story section.
    setBeat(id, { story: hash ? story : "rest", writeHash: Boolean(hash) });
  }, [study.slug, toc]);
          ...
          <div className="case-article">
            ...
            {study.productHref ? (
              ...
              >
                ...
                {canEmbedProduct(study.slug) ? (
                  <>
                    <p>
                      Explore the working product below without leaving this case. The frame is the
 live app - not a mock - then open it full-size if you want the full journey.
                    </p>
                    <ProductPreview
                      href={study.productHref}
                      label={study.productLabel ?? "Open product"}
                      slug={study.slug}
                    />
                  ...
                  <>
                    <p>
                      {study.slug === "daylight"
 ? "Daylight is a signed macOS menu-bar app. The site explains Gatekeeper, schedules, and certainty - open it for the full product story."
                        : "The working app is live, not a mock. Open it, use it, then come back to the case if you want the product decisions behind it."}
                    </p>
                    <div className="hero-actions">
                      <ProductLink className="btn btn-ink" href={study.productHref}>
                        {study.productLabel ?? "Open product"} ↗
...
function applyStory(slug: string, story: typeof visual.story) {
 ...
 // Diff is the teaching default - before/live failure is reserved for story "broken".
    if (slug === "valuation") visual.reconcile = 0.55;
    ...
    if (slug === "daylight") visual.day = 0.55;
