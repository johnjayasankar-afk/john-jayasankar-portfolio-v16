...
- Stories are HTML and CSS drawn with the site's own tokens, so they stay crisp
  ...
  shows: floating notes above 640px, a denser window below 430px.
...
- `http://localhost:4401/_stories?only=iport&step=1` shows one story at one
  phase in every frame size; `?auto=1` plays them all. Ideas are there too, at
  their card sizes: `?only=note-routing`, `?only=p-ladder`.
