...
def showcase():
    ...
    for i, t in enumerate(H['show']):
        ...
        tabs.append('<button type="button" class="seg__btn show__tab" role="tab" id="show-tab-%s" aria-controls="show-%s" '
                    ...
                    % (key, key, 'true' if sel else 'false', '0' if sel else '-1', esc(t['tab'])))
...
def approach():
    ...
    body = (pagehead('Approach', A['kicker'], A['h1'], A['lede'], CONTINUE)
            ...
            + ('<section class="sect" id="ladder" data-locus data-label="Autonomy ladder" aria-labelledby="ladder-h">'
               '<div class="wrap ladder"><div class="ladder__copy" data-reveal>%s<h2 class="h2" id="ladder-h">%s</h2>'
               '<p class="shead__lede">%s</p>%s</div><div class="ladder__vis" data-reveal style="--rd:1"><p class="ladder__locus" data-ladder-locus>%s</p>'
               '<ol class="rungs" aria-label="Autonomy ladder">%s</ol>'
               '<p class="ladder__read" data-ladder-read aria-live="polite"><b>%s</b> %s</p></div></div>'
               '<div class="wrap">%s</div></section>\n') % (
                slabel(L['n'], L['kicker']), esc(L['h2']), esc(L['lede']), tlink(L['link'][0], L['link'][1]),
