...
It also requests the same-origin files the pages point at, which the build
cannot check for itself: the build knows a link resolves to a path it wrote,
not that the file behind it is there and served. The resume PDF is the one that
matters. It is linked from six places, it is not generated by the build, and
nothing else would notice if it went missing.
...
BOT_WALLED = {
    ...
    'www.linkedin.com': '999, which is LinkedIn refusing any non-browser client. '
                        'A logged-out browser gets the auth wall, which echoes the '
                        'profile path back in sessionRedirect, so the URL is right; '
                        'a signed-in visitor, which is most people clicking this, '
                        'lands on the profile. Checked 26 September 2026',
}
...
def outbound(site=None):
    """Every URL the built site points at, with the pages that point at it.

    Absolute links go out as written. Same-origin paths are resolved against
    `site` so the file behind them is actually requested: a document, a PDF, an
    image. Page routes are left to the build, which already refuses to finish on
    a dangling internal link.
    """
    seen = {}
    for path in html_files():
        ...
        for url in re.findall(r'(?:href|src|content)="(https?://[^"]+)"', body):
            seen.setdefault(url.replace('&amp;', '&'), set()).add(rel)
        if not site:
            continue
        for ref in re.findall(r'(?:href|src)="(/[^"#?]+\.[a-z0-9]{2,5})"', body):
            if ref.endswith('.html'):
                continue
            seen.setdefault(site.rstrip('/') + ref, set()).add(rel)
    return seen
...
def main():
    ...
    if want('--links'):
        print('LINKS')
        bad = check_links(os.environ.get('SITE_ORIGIN') or 'http://127.0.0.1:4400'
                          if os.environ.get('CHECK_ASSETS', '1') == '1' else None)
        print()
    if want('--hops'):
        print('HOPS')
